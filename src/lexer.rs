...
impl Lexer {
    ...
    pub fn tokenize(&mut self) -> Vec<TokenInfo> {
        ...
        while !self.is_at_end() { // Loop until the end of input
            self.skip_whitespace(); // Skip whitespace characters
            
            if self.is_at_end() { // Check again in case we reached the end
                break; // Exit the loop
            }
            ...
            match self.current_char() { // Match on the current character
                '`' => {
                    self.advance();
                ...
                '-' => {
                    ...
                    if self.current_char() == '>' {
                        self.advance();
                        tokens.push(TokenInfo::new(Token::Arrow, line, column)); // Add an Arrow token
                    } else {
                        tokens.push(TokenInfo::new(Token::Minus, line, column)); // Add a Minus token
                    }
                }
                '*' => {
                    self.advance();
                ...
                '/' => {
                    self.advance();
                    if self.current_char() == '/' { // Check for comment
                        // Skip comment until end of line
                        while !self.is_at_end() && self.current_char() != '\n' {
                            self.advance(); // Skip each character in the comment
                        }
                    } else {
                        tokens.push(TokenInfo::new(Token::Slash, line, column)); // Add a Slash token
                    }
                }
                '=' => {
                    self.advance();
                    if self.current_char() == '=' {
                        self.advance();
                        tokens.push(TokenInfo::new(Token::Equal, line, column)); // Add an Equal token (==)
                    } else {
                        tokens.push(TokenInfo::new(Token::Assign, line, column)); // Add an Assign token (=)
                    }
                }
                '!' => {
                    self.advance();
                ...
                '<' => {
                    ...
                    if self.current_char() == '=' {
                        self.advance();
                        tokens.push(TokenInfo::new(Token::LessEqual, line, column)); // Add a LessEqual token (<=)
                    } else if self.current_char() == '-' {
                        self.advance();
                        tokens.push(TokenInfo::new(Token::ArrowLeft, line, column)); // Add a ArrowLeft token (<-)
                    } else {
                ...
                '>' => {
                    ...
                    if self.current_char() == '=' {
                        self.advance();
                        tokens.push(TokenInfo::new(Token::GreaterEqual, line, column)); // Add a GreaterEqual token (>=)
                    } else {
                        tokens.push(TokenInfo::new(Token::Greater, line, column)); // Add a Greater token (>)
                    }
                }
                '(' => {
                    self.advance();
                    tokens.push(TokenInfo::new(Token::LeftParen, line, column)); // Add a LeftParen token
                }
                ')' => {
                    self.advance();
                    tokens.push(TokenInfo::new(Token::RightParen, line, column)); // Add a RightParen token
                }
                '{' => {
                    self.advance();
                    tokens.push(TokenInfo::new(Token::LeftBrace, line, column)); // Add a LeftBrace token
                }
                '}' => {
                    self.advance();
                    tokens.push(TokenInfo::new(Token::RightBrace, line, column)); // Add a RightBrace token
                }
                '[' => {
                    self.advance();
                    tokens.push(TokenInfo::new(Token::LeftBracket, line, column)); // Add a LeftBracket token
                }
                ']' => {
                    self.advance();
                    tokens.push(TokenInfo::new(Token::RightBracket, line, column)); // Add a RightBracket token
                }
                ';' => {
                    self.advance();
                    tokens.push(TokenInfo::new(Token::Semicolon, line, column)); // Add a Semicolon token
                }
                ',' => {
                    self.advance();
                ...
                '"' => {
                    ...
                    tokens.push(TokenInfo::new(Token::String(string_literal), line, column)); // Add a String token
                }
                c if c.is_ascii_digit() => {
                    let number = self.scan_number(); // Parse a number literal
                    tokens.push(TokenInfo::new(Token::Number(number), line, column)); // Add a Number token
                }
                ...
                _ => {
                    panic!("Unexpected character '{}' at line {}, column {}", self.current_char(), line, column); // Error for unknown character
                }
            }
        }
    ...
    fn scan_string(&mut self) -> String {
        ...
        if self.is_at_end() {
            panic!("Unterminated string at line {} column {}", self.line, self.column); // Error if string not closed
        }
    ...
    fn scan_number(&mut self) -> f64 {
        ...
        value.parse().unwrap_or_else(|_| {
            panic!("Invalid number '{}' at line {}, column {}", value, self.line, self.column); // Error if not a valid number
        })
    }
